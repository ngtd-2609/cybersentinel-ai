...
required_files=(
  ...
  docs/releases/v1.2.0-handoff.md
  docs/releases/v1.2.0-state.json
...
python3 -m json.tool docs/releases/v1.2.0-state.json >/dev/null
python3 - <<'PY'
import json
from pathlib import Path

state = json.loads(Path("docs/releases/v1.2.0-state.json").read_text())
required = {
    "clean_git_tree",
    "fresh_clone",
    "one_command_startup",
...
if state["release"]["version"] != "1.2.0":
    raise SystemExit("Release state must describe v1.2.0")

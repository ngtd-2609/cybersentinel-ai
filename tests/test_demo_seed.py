...
from cybersentinel_ai.db.models import AlertRule, DetectionEvent, Incident, IncidentTimeline, User
...
def test_demo_seed_is_idempotent_and_uses_restricted_account(database_factory):
    ...
    with database_factory() as database:
        ...
        assert database.scalar(select(func.count()).select_from(AlertRule)) == 10
        labels = set(database.scalars(select(AlertRule.label_pattern)).all())
        assert {"RANSOMWARE", "SSH-BRUTE-FORCE", "PORT-SCAN", "WEB-ATTACK", "C2-TRAFFIC"} <= labels

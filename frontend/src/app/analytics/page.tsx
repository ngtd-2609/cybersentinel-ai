    ...
    <div className="flex min-h-screen bg-slate-50 text-slate-950">
      ...
      <div className="min-w-0 flex-1">
        ...
        <main className="mx-auto max-w-[1600px] p-5 md:p-8">
          <header className="mb-8"><p className="mb-2 text-xs font-semibold uppercase tracking-[0.16em] text-cyan-700">{t("Detection intelligence")}</p><h1 className="text-3xl font-semibold tracking-tight">{t("Security Analytics")}</h1><p className="mt-2 text-sm text-slate-500">{t("Live severity, risk and attack-pattern analysis from stored detection events.")}</p></header>
          {isError ? <p className="rounded-xl border border-red-200 bg-red-50 p-4 text-red-700">{t("Unable to load analytics. Try again.")}</p> : (
            <>
              <section className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {isLoading ? Array.from({ length: 4 }).map((_, index) => <Skeleton key={index} className="h-28 rounded-xl" />) : metrics.map((metric) => (
                  <Card key={metric.label}><CardContent className="flex items-center justify-between p-5"><div><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">{t(metric.label)}</p><p className="mt-2 text-3xl font-semibold">{metric.value}</p></div><metric.icon className="size-7 text-cyan-600" /></CardContent></Card>
                ))}
              </section>
              <section className="grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(340px,1fr)]">
                <Card><CardHeader><CardTitle>{t("24-hour event trend")}</CardTitle></CardHeader><CardContent><SecurityEventsChart /></CardContent></Card>
                <Card><CardHeader><CardTitle>{t("Attack distribution")}</CardTitle></CardHeader><CardContent><AttackTypesChart /></CardContent></Card>
              </section>
              <Card className="mt-6"><CardHeader><CardTitle>{t("Top threat sources")}</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                ...
                )) : <p className="text-sm text-slate-500">{t("No source intelligence is available. This does not confirm that no threats exist.")}</p>}
              </CardContent></Card>
            </>
          )}
        </main>
